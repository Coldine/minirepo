...
function draw() {
  ...
  for (let i = 0; i < config[global_position].tableA.sobres; i++)
    contentA.push(TA_S);
  for (let i = 0; i < config[global_position].tableB.cartas; i++)
    contentB.push(TB_C);
  for (let i = 0; i < config[global_position].tableB.sobres; i++)
    contentB.push(TB_S);
...
document
  .getElementById("modelo-carta")
  .setAttribute("src", `data:image/png;base64,${carta}`);
document
  .getElementById("modelo-sobre")
  .setAttribute("src", `data:image/png;base64,${sobre}`);
